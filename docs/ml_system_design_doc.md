...
**FYI:** Это Pet-проект, данные взяты с хакатона, проводимого Avito.tech. Я постарался рассмотреть кейс и с продуктовой точки зрения, поэтому по большей части это моя вольная интерпретация того, как могло бы быть.
---
...
- **Бизнес-цель**:  
  Платформа Avito.tech стремится повысить эффективность рекламных кампаний, предлагая пользователям наиболее релевантные объявления. Это позволит увеличить ключевые бизнес-метрики: CTR (Click-Through Rate) и ROI (Return on Investment), улучшив удовлетворенность пользователей и повысив доходы рекламодателей. Основная цель — разработать ML-модель, которая будет прогнозировать вероятность клика на рекламу, учитывая поведенческие данные пользователей, характеристики рекламных кампаний и другие связанные параметры.

- **Почему станет лучше, чем сейчас, от использования ML**:  
  Использование ML-моделей для предсказания вероятности клика обеспечит:
  * Персонализированные рекомендации, повышающие удовлетворенность пользователей.
  * Сокращение неэффективных показов рекламы, что приведет к снижению затрат и увеличению ROI рекламодателей.
  * Повышение конкурентоспособности платформы за счет улучшения качества рекламных сервисов.

- **Что будем считать успехом итерации с точки зрения бизнеса (Product Owner)**:  
  Успехом будет считаться:
  * Результаты A/B тестирования демонстрируют статистически значимое увеличение CTR на 5% (95% доверительный интервал).
  * Затраты на рекламу снижаются благодаря сокращению нецелевых показов, что улучшает ROI.
  * Решение интегрировано в текущую инфраструктуру платформы и демонстрирует время ответа менее 500 мс.
  * Модель достигает порога ROC AUC ≥ 0.6, подтверждая свою точность в прогнозировании кликов.
...
- **Краткое описание БТ и ссылки на детальные документы с бизнес-требованиями**:  
  Основное бизнес-требование — разработка модели, предсказывающей вероятность клика на рекламное объявление. Модель должна быть адаптирована для работы с анонимизированными пользовательскими данными в соответствии с требованиями GDPR и обеспечивать высокую производительность в условиях продакшена.

- **Use case**
  При выборе рекламного объявления для показа пользователю внешняя рекламная система Avito обращается к ML-сервису, получает от него предсказания вероятностей того, что пользователь кликнет на соответствующие рекламные баннеры, и использует их при принятии окончательного решения. Подробное описание формата входных и выходных данных можно найти в разделе 4. Внедрение.

- **Бизнес-ограничения**:  
  1. Модель должна работать исключительно с анонимизированными данными пользователей, без нарушения принципов конфиденциальности.
  2. Скорость предсказания должна обеспечивать обработку запросов в режиме реального времени (задержка не более 500 мс).
  3. Решение должно быть реализовано на CPU и не требовать использования GPU для первой версии.
  4. Текущий объем исторических данных ограничен 3 неделями, что задает определенные рамки для обучения модели.

- **Что мы ожидаем от конкретной итерации**:  
  1. Будет разработан рабочий прототип ML-модели, который валидируется по метрике ROC AUC.
  2. Прототип продемонстрирует улучшение точности предсказаний по сравнению с бейзлайном (например, на основе предположения о повторных кликах).
  3. Модель успешно пройдет офлайн-валидацию и подготовку к пилотному использованию в инфраструктуре платформы.
- **Описание бизнес-процесса пилота**:  
  В рамках пилота будет использоваться модель градиентного бустинга, интегрированная с внешним сервисом рекомендации рекламы Avito.tech. Новый ML-сервис будет предсказывать вероятность кликов, на основе которых формируется список релевантных объявлений.  

  Эффективность решения будет оцениваться с помощью **A/B тестирования**, где контрольная группа будет использовать базовое решение (без ML), а пилотная — новый ML-сервис. Ключевые метрики: CTR, ROI, время отклика и использование ресурсов.
...
- **Что считаем успешным пилотом? Критерии успеха и возможные пути развития проекта (Product Owner)**:  
  ...
  * Порог ROC AUC ≥ 0.6 превышен на валидационных данных.
  * Результаты A/B тестирования показывают статистически значимое увеличение CTR и ROI.
  * Модель стабильно работает в условиях продакшена, соответствуя техническим требованиям (время отклика, использование памяти).
...
- **На закрытие каких БТ подписываемся в данной итерации**:  
  * Создание модели, которая предсказывает вероятность клика на основе исторических данных о взаимодействиях пользователей с рекламой и характеристиках рекламных кампаний.
  * Проведение валидации модели и проверка ее качества на основе установленных метрик (ROC AUC, Precision, Recall).

- **Что не будет закрыто**:  
  * Проведение пилота (A/B тестирования).
  * Полная интеграция модели в инфраструктуру API.
  * Создание системы автоматического дообучения модели.

- **Описание результата с точки зрения качества кода и воспроизводимости решения**:  
  - Код должен соответствовать PEP8, иметь докстринги, и проходить проверки линтеров.
  - Решение должно быть воспроизводимым, с использованием Docker контейнеров для упрощения развертывания модели.
...
- **Описание всех общих предпосылок решения, используемых в системе**:  
  - Данные о взаимодействиях пользователей с рекламой являются достаточными для предсказания вероятности клика.
  - Модель будет обучена на данных за последние 3 недели, что обеспечивает актуальность предсказаний.
  - Модель будет функционировать в продакшене на CPU, без необходимости использования GPU, что соответствует текущим бизнес-ограничениям.
  - ML-сервис будет использоваться как часть системы рекомендации Avito.tech для улучшения релевантности рекламы, обеспечивая предсказания с минимальной задержкой (не более 50 мс).
...
- **Задача**: Разработка ML-модели, которая прогнозирует вероятность клика на рекламное объявление (бинарная классификация).  
- **Входные данные**:  
  - Исторические данные о взаимодействиях пользователей с рекламными объявлениями.
  - Характеристики рекламных кампаний (например, категория, бюджет, целевая аудитория).  
- **Выходные данные**:  
  - Вероятность клика (значение от 0 до 1) для каждого объявления.  

Цель модели — повысить CTR (Click-Through Rate) и ROI (Return on Investment) рекламных кампаний за счет повышения релевантности показов объявлений.
...
* Большое количество данных может потенциально привести к долгому времени обучения моделей и большим затратам оперативной памяти
...
- **FastAPI + Gunicorn**: Поддерживает 4 реплики воркеров, которые справляются с текущей нагрузкой в 250+ RPS при медианной задержке 11 мс.
...
4. **Легкость развёртывания**: Все компоненты решения оформлены в виде докер-контейнеров и собраны в едином docker compose файле, что позволяет запустить их все с помощью команды `docker compose up --build`
...
- Доступность сервиса 99.9% времени в году.
- Время восстановления сервиса после аварии не более 24 часов.
...
### 4.9 Дообучение модели
Дообучение модели в перспективе будет происходить с помощью Airflow: свежие данные будут доставаться по расписанию из БД основного сервиса рекламы Avito, на них будет обучаться модель аналогичным образом, который описан в этом документе, с логированием процесса обучения и сохранением всех артефактов (включая обновленную модель) в MLFlow, из которого свежая модель будет попадать в ML-сервис.

### 4.10 Потенциальные улучшения инфраструктуры
* Использование Airflow для обучения на новых данных по расписанию
* Добавление онлайн feature store
* Использование нейросетевых моделей с вытекающим использованием GPU
* Использование Triton Inference Server для обеспечения лёгкой работы в режиме батчинга
* Кэширование запросов с помощью Redis
* Добаление Nginx для балансировки нагрузки на нескольких машинах
* Переход на k8s для лучшей оркестрации работы контейнеров
